...
import { useElectricityDetails } from '@/api/power/useElectricityDetails'
...
export function SolarProduction() {
  ...
  useEffect(() => {
    if (solarData) {
      const temp = []
      for (let i = 0; i < solarData.powerCurve.xAxis.length; i++) {
        if (solarData.powerCurve.activePower[i] === '-') continue
        temp.push({
          hour: solarData.powerCurve.xAxis[i],
          power: solarData.powerCurve.activePower[i],
        })
      }
      setChartData(temp)
    }
  ...
  if (isSolarError) {
    return <span>Error: {solarError.message}</span>
  }
  ...
  return (
    <>
      ...
      <Drawer>
        ...
        <DrawerContent>
          <DrawerHeader className={'grid grid-cols-2 gap-3'}>
            <SolarUnitCard
